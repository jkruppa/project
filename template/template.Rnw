...
\item Sie stellen in Ihrer Pr{\"a}sentation Ihre selbst erstellte R Shiny Web
  App vor. 
\end{enumerate}
...
einen Datensatz von Ihrem Interesse runter.

\begin{enumerate}
\item Erstellen Sie eine R Shiny Web App aus den Daten.
\item Sie geben die R Shiny Web App als ZIP-Datei beinhaltend alle
...
\item Bauen Sie aus den selbst erstellten Daten die fotografierte Abbildung in
  \texttt{ggplot()} nach!
\item Geben Sie den Code und die Abbildung in R Quarto ab!
\end{enumerate}

\subsection*{Zerforschung II}

\begin{enumerate}
\item Begeben Sie sich in die Bibliothek in Haste. Suchen Sie sich drei
  Abschlussarbeiten aus.
\item Erstellen Sie eine deskriptive {\"U}bersicht {\"u}ber den Umfang der drei
  Abschlussarbeiten!
\item Sehen Sie folgende Fragen als Inspiration. Wie viele Referenzen
...
\item Fasse Sie Ihre {\"U}bersicht in maximal drei Seiten zusammen.
\end{enumerate}

\subsection*{Reflexion I}

Besuchen Sie die Internetseite https://www.bigbookofr.com/ und w{\"a}hlen Sie
eines der {\"u}ber 300 B{\"u}cher aus.

\begin{enumerate}
\item Schreiben Sie eine zweiseitige Zusammenfassung {\"u}ber das Buch!
\end{enumerate}
